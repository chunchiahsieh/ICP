...
public sealed class IlcArurWriteService : IIlcArurWriteService
{
    ...
    private static void Validate(IlcRtArurHeader r) { var f=new List<string>(); Check("WHCode",r.WhCode,4,f); Check("TETPO",r.TetPo,30,f); Check("Attachment",r.Attachment,1000,f); Check("Subject",r.Subject,50,f); if(f.Count>0) throw new InvalidOperationException("ARUR validation failed: "+string.Join("; ",f)); }
