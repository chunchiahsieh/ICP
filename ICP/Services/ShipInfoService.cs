...
public class ShipInfoService : IShipInfoService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await _repository.SearchHeadersAsync(criteria, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var currentCaseStatus = caseType == ShipInfoCaseTypes.Deposit
            ? header.DepositCaseStatus
            : header.ArurCaseStatus;
    ...
    private async Task<ShipInfoCaseCreateResult> ResendFailedOutboxAsync(
        IcpHeader header,
        string headerRowKey,
        string invoiceKey,
        string caseType,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (caseType == ShipInfoCaseTypes.Deposit && !permission.Deposit)
        {
            throw new ShipInfoBusinessException("Deposit case cannot be created in current status.");
        }

        if (caseType == ShipInfoCaseTypes.Arur && !permission.Arur)
        {
            throw new ShipInfoBusinessException("ARUR case cannot be created in current status.");
        }

        var requeued = await _outboxRepository.TryRequeueFailedAsync(invoiceKey, caseType, userName, cancellationToken);
        if (!requeued)
        {
            throw new ShipInfoBusinessException(
                "No Failed integration outbox event to resend for this case.");
        }

        var caseNo = caseType == ShipInfoCaseTypes.Deposit ? header.Deposit : header.RtNo;
        var auditLog = CreateAuditLog(
            "Header",
            headerRowKey,
            invoiceKey,
        ...
        LogOperation(
            caseType == ShipInfoCaseTypes.Deposit ? "DepositResend" : "ARURResend",
    ...
    private async Task ApplyOutboxFailedFlagsAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var headerKeys = items
            .Select(item =>
            {
                if (item.TryGetValue("HeaderKey", out var key) && key is not null)
                {
                    return Convert.ToString(key) ?? string.Empty;
                }
        ...
        var flags = await _outboxRepository.GetFailedFlagsByHeaderKeysAsync(headerKeys, cancellationToken);
        foreach (var item in items)
        {
            var key = item.TryGetValue("HeaderKey", out var raw) ? Convert.ToString(raw) : null;
            flags.TryGetValue(key ?? string.Empty, out var flag);
            ShipInfoRowDtoMapper.ApplyOutboxFailedFlags(item, flag);
        }
    }
